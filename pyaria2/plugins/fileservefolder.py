...
class Fileservefolder(object):
    ...
    def check_url(self, request):
        ...
        for target in request.targets:
            if re.findall(URL_SCHEME, target.link):
                debug("Fileservefolder: %s is a fileserver folder url" % target.link)
                have_results = True
    ...
    def process(self, link):
        ...
        table = document.xpath("//table[@class='file_list']")[0]
        links = []
        for a in table.xpath(".//a[@class='sheet_icon wbold']"):
            link = "http://www.fileserve.com%s" % a.get("href")
            debug("Fileservefolder: link found %s" % link)
            links.append( link )
        return links
